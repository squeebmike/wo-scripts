...
// v14: all Pokémon keep baked normal + shiny fallback palettes, then selected Pokémon can be corrected from pre-extracted palette JSON.
// No canvas/image sampling: palettes are converted into contrast-safe UI roles for readable Webflow theming.
var LS='wo-v14-static-pokemon-contrast-palette';
...
function smartPalette(o){
  ...
  var bg=o.base||(isPokemon?mix('#05070f',pri,lum(pri)<.2?.38:.22):(lum(pri)<.16?mix('#05070f',pri,.42):mix('#060914',pri,.24)));
  if(lum(bg)>.22)bg=mix(bg,'#000000',.55);
  var surf=o.surface||readableSurface(bg,.11);
  var surfAlt=o.surfaceAlt||readableSurface(bg,.18);
  var txt=onBg(bg);
  var txtSub=rgba(txt,.82),txtMut=rgba(txt,.62);
  var bdr=rgba(txt,.16),bdrStr=rgba(txt,.30);
  var da=safeAccent(acc,bg,4.5);
  var safeHi=safeAccent(hi,bg,3.6);
  var navBg=o.navBg||(isPokemon?mix(bg,pri,.42):pri);
  if(contrast(onBg(navBg),navBg)<4.5)navBg=bg;
  var heroBg=mix(bg,pri,.44);
  if(lum(heroBg)>.25)heroBg=mix(heroBg,'#000000',.42);
  var footBg=mix(bg,'#000000',.26);
...
function buildModal(){
  ...
  var sub=document.createElement('div');sub.id='wo-sub';sub.textContent='Teams, MTG mana palettes, and baked Pokémon/shiny palettes update the site with readable contrast.';
