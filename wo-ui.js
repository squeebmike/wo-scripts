...
function polishNavigation(){
  ...
  function wireDropdown(dropdown,index){
    ...
    dropdown.addEventListener('mouseenter',function(){if(innerWidth>991){clearTimeout(timer);toggle.classList.add('w--open');list.classList.add('w--open');sync();}});dropdown.addEventListener('mouseleave',function(){if(innerWidth>991)timer=setTimeout(function(){if(dropdown.dataset.mpClickOpen==='true'||dropdown.contains(document.activeElement))return;toggle.classList.remove('w--open');list.classList.remove('w--open');sync();},120);});
