...
function polishNavigation(){
  ...
  function wireDropdown(dropdown,index){
    ...
    toggle.addEventListener('click',function(event){if(innerWidth<=991)return;event.preventDefault();event.stopImmediatePropagation();var willOpen=!list.classList.contains('w--open');dropdown.parentElement&&Array.prototype.forEach.call(dropdown.parentElement.querySelectorAll('.w-dropdown'),function(other){if(other===dropdown)return;var otherToggle=other.querySelector('.w-dropdown-toggle');var otherList=other.querySelector('.w-dropdown-list');if(otherToggle&&otherList){otherToggle.classList.remove('w--open');otherList.classList.remove('w--open');otherToggle.setAttribute('aria-expanded','false');}});toggle.classList.toggle('w--open',willOpen);list.classList.toggle('w--open',willOpen);sync();},true);
    dropdown.addEventListener('mouseenter',function(){if(innerWidth>991){clearTimeout(timer);toggle.classList.add('w--open');list.classList.add('w--open');sync();}});dropdown.addEventListener('mouseleave',function(){if(innerWidth>991)timer=setTimeout(function(){if(dropdown.contains(document.activeElement))return;toggle.classList.remove('w--open');list.classList.remove('w--open');sync();},120);});
