...
function polishNavigation(){
  ...
  function wireDropdown(dropdown,index){
    ...
    dropdown.addEventListener('focusin',function(){if(innerWidth>991){toggle.classList.add('w--open');list.classList.add('w--open');sync();}});dropdown.addEventListener('focusout',function(){if(innerWidth>991)setTimeout(function(){if(!dropdown.contains(document.activeElement)){toggle.classList.remove('w--open');list.classList.remove('w--open');sync();}},0);});
  }
  function mount(){
