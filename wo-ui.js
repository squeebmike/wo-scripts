...
window.ManaPocketInventory={get:getManaPocketInventory};
...
WO.applyTheme=function(team,colorway){
  ...
  var css=
  ':root{--wo-brand-primary:'+p.rawPri+';--wo-brand-secondary:'+p.rawSec+';--wo-primary:'+p.pri+';--wo-secondary:'+p.sec+';--wo-accent:'+p.acc+';--wo-on-accent:'+p.onAcc+';--wo-highlight:'+p.hi+';--wo-button:'+p.btn+';--wo-on-button:'+p.onBtn+';--wo-button-hover:'+p.btnHov+';--wo-on-button-hover:'+p.onBtnHov+';--wo-bg:'+p.bg+';--wo-surface:'+p.surf+';--wo-surface-alt:'+p.surfAlt+';--wo-text:'+p.txt+';--wo-text-sub:'+p.txtSub+';--wo-text-muted:'+p.txtMut+';--wo-border:'+p.bdr+';--wo-border-strong:'+p.bdrStr+';--wo-nav-bg:'+p.navBg+';--wo-on-nav:'+p.onNav+';--wo-nav-highlight:'+p.navHi+';--wo-hero-bg:'+p.heroBg+';--wo-on-hero:'+p.onHero+';--wo-footer-bg:'+p.footBg+';--wo-footer-text:'+p.footTxt+';--wo-footer-highlight:'+p.footHi+';--wo-shadow:'+p.shadow+';--wo-chip-bg:'+p.chipBg+';--wo-chip-text:'+p.chipTxt+';--wo-focus:'+p.focus+';}'+
  ...
  '.text-white,.Text-White{color:'+p.onHero+' !important;}'+
  ...
  '.underline-link.light,.underline-link.Light{color:'+p.onHero+' !important;}'+
...
function polishNavigationLegacy(){
  ...
  var SHOP_LINKS=[
    ...
    {label:'Sports',href:'/shop?cat=sports-cards',image:'https://s3.amazonaws.com/webflow-prod-assets/65b15ee0228d06647ca7e4ce/660ba51ad445f22b83625c86_Walk-off-sportscards-toploader.webp'}
  ];
    ...
    if(!nav.dataset.mpScrollClose){
      ...
      window.addEventListener('scroll',function(){
        ...
        if(window.innerWidth<=991&&menuButton&&Math.abs(nextScrollY-lastScrollY)>8){
          menuButton.click();
          if(nextScrollY>lastScrollY)requestAnimationFrame(function(){nav.classList.add('is-hidden');});
        }
...
function polishNavigation(){
  ...
  var SHOP_LINKS=[
    {slug:'all',label:'All',href:'/shop',image:BRAND_LOGO,meta:'Everything in the case'},
  ...
  var COOL_LINKS=[
    ...
    {slug:'fan-club',label:'Fan Club',href:'/fan-club',image:BRAND_LOGO,meta:'Your membership card'}
  ];
  function categorySlug(item){
    var value=String(item&&item.category||'').toLowerCase();
  ...
  function inventoryScore(item){
    var words=[item.name,item.category,item.set,item.variant,item.condition].join(' ').toLowerCase();
    var updated=new Date(item.addedAt||item.updatedAt||0).getTime()||0;
    var freshness=Math.max(0,30-Math.floor((Date.now()-updated)/86400000));
    var score=freshness*4+Math.min(120,Number(item.price)||0);
    if(/graded|slab|signed|variant|rare|sealed|booster|bundle|collector|key/.test(words))score+=80;
    return score;
  }
  function selectInventory(items,slug,limit){
    var filtered=items.filter(function(item){return item&&/^https?:\/\//i.test(item.image||'')&&(slug==='all'||categorySlug(item)===slug);});
    filtered.sort(function(a,b){return inventoryScore(b)-inventoryScore(a);});
    var seen={};return filtered.filter(function(item){if(seen[item.image])return false;seen[item.image]=true;return true;}).slice(0,limit||3);
  }
  function merchandise(items,fallback){
    var wrap=document.createElement('span');wrap.className='mp-counter-merch';wrap.setAttribute('aria-hidden','true');
    var sources=items&&items.length?items.map(function(item){return item.image;}):[fallback];
    sources.slice(0,3).forEach(function(src,index){
      var frame=document.createElement('span');frame.className='mp-counter-item mp-counter-item--'+(index+1);
      var image=document.createElement('img');image.alt='';image.loading='lazy';image.decoding='async';image.src=src;
      image.addEventListener('error',function(){frame.hidden=true;});frame.appendChild(image);wrap.appendChild(frame);
    });
    return wrap;
  }
  function buildShopZone(item,items,compact){
    var link=document.createElement('a');link.className=compact?'mp-mobile-case-card':'mp-counter-zone';link.href=item.href;link.dataset.mpCategory=item.slug;
    link.appendChild(merchandise(selectInventory(items||[],item.slug,compact?1:3),item.image));
    var copy=document.createElement('span');copy.className='mp-counter-copy';var label=document.createElement('strong');label.textContent=item.label;copy.appendChild(label);var meta=document.createElement('span');meta.textContent=item.meta;copy.appendChild(meta);link.appendChild(copy);
    if(item.slug==='pokemon'){var sticker=document.createElement('span');sticker.className='mp-counter-sticker';sticker.textContent='NEW';sticker.setAttribute('aria-hidden','true');link.appendChild(sticker);}
    return link;
  }
  function preserveRoutes(dropdown,items){
    var existing=dropdown?Array.prototype.map.call(dropdown.querySelectorAll('a'),function(link){return{label:link.textContent.trim(),href:link.getAttribute('href')};}):[];
    return items.map(function(item){var match=existing.find(function(link){return link.label.toLowerCase()===item.label.toLowerCase();});var copy={};Object.keys(item).forEach(function(key){copy[key]=item[key];});if(match&&match.href)copy.href=match.href;return copy;});
  }
  ...
  function wireDropdown(dropdown,index){
    if(!dropdown)return;var toggle=dropdown.querySelector('.w-dropdown-toggle');var list=dropdown.querySelector('.w-dropdown-list');if(!toggle||!list)return;if(!list.id)list.id='mp-counter-menu-'+index;toggle.setAttribute('aria-controls',list.id);toggle.setAttribute('aria-haspopup','true');
    function sync(){toggle.setAttribute('aria-expanded',String(list.classList.contains('w--open')));}sync();new MutationObserver(function(){if(dropdown.dataset.mpClickOpen==='true'&&!list.classList.contains('w--open')){toggle.classList.add('w--open');list.classList.add('w--open');}sync();}).observe(list,{attributes:true,attributeFilter:['class']});var timer;
    toggle.addEventListener('click',function(event){if(innerWidth<=991)return;event.preventDefault();event.stopImmediatePropagation();var willOpen=dropdown.dataset.mpClickOpen!=='true';dropdown.parentElement&&Array.prototype.forEach.call(dropdown.parentElement.querySelectorAll('.w-dropdown'),function(other){if(other===dropdown)return;var otherToggle=other.querySelector('.w-dropdown-toggle');var otherList=other.querySelector('.w-dropdown-list');if(otherToggle&&otherList){other.dataset.mpClickOpen='false';otherToggle.classList.remove('w--open');otherList.classList.remove('w--open');otherToggle.setAttribute('aria-expanded','false');}});dropdown.dataset.mpClickOpen=willOpen?'true':'false';toggle.classList.toggle('w--open',willOpen);list.classList.toggle('w--open',willOpen);sync();},true);
    function keepHoverOpen(){if(innerWidth>991)clearTimeout(timer);}
    function scheduleHoverClose(){if(innerWidth>991)timer=setTimeout(function(){if(dropdown.dataset.mpClickOpen==='true'||dropdown.contains(document.activeElement)||toggle.matches(':hover')||list.matches(':hover'))return;toggle.classList.remove('w--open');list.classList.remove('w--open');sync();},320);}
    dropdown.addEventListener('mouseenter',function(){if(innerWidth>991){keepHoverOpen();toggle.classList.add('w--open');list.classList.add('w--open');sync();}});dropdown.addEventListener('mouseleave',scheduleHoverClose);toggle.addEventListener('mouseenter',keepHoverOpen);list.addEventListener('mouseenter',keepHoverOpen);list.addEventListener('mouseleave',scheduleHoverClose);
    dropdown.addEventListener('focusin',function(event){if(innerWidth>991&&event.target!==toggle){toggle.classList.add('w--open');list.classList.add('w--open');sync();}});dropdown.addEventListener('focusout',function(){if(innerWidth>991)setTimeout(function(){if(!dropdown.contains(document.activeElement)){toggle.classList.remove('w--open');list.classList.remove('w--open');sync();}},0);});
  }
  function mount(){
    ...
    var counterHydrated=false;function hydrateCounterInventory(){if(counterHydrated)return;counterHydrated=true;getManaPocketInventory().then(function(items){buildDesktopShop(shopList,items);buildDesktopCool(coolList,coolItems);if(menu)buildMobileDrawer(nav,menu,coolItems,items);}).catch(function(){});}nav.addEventListener('pointerenter',hydrateCounterInventory,{once:true,passive:true});nav.addEventListener('focusin',hydrateCounterInventory,{once:true});
    ...
    document.addEventListener('click',function(event){if(innerWidth<=991||nav.contains(event.target))return;dropdowns.forEach(function(dropdown){var toggle=dropdown.querySelector('.w-dropdown-toggle');var list=dropdown.querySelector('.w-dropdown-list');dropdown.dataset.mpClickOpen='false';if(toggle&&list){toggle.classList.remove('w--open');list.classList.remove('w--open');toggle.setAttribute('aria-expanded','false');}});});
    if(!nav.dataset.mpScrollClose){nav.dataset.mpScrollClose='true';var lastScrollY=window.scrollY;window.addEventListener('scroll',function(){var nextScrollY=window.scrollY;var menuButton=nav.querySelector('.w-nav-button.w--open');if(innerWidth<=991&&menuButton&&Math.abs(nextScrollY-lastScrollY)>8){menuButton.click();document.body.classList.remove('mp-mobile-nav-open');if(nextScrollY>lastScrollY)requestAnimationFrame(function(){nav.classList.add('is-hidden');});}lastScrollY=nextScrollY;},{passive:true});}
  }
  if(document.body)mount();else document.addEventListener('DOMContentLoaded',mount,{once:true});
}

function buildReceiptFooter(){
  var BRAND_LOGO='https://cdn.prod.website-files.com/65b15ee0228d06647ca7e4ce/6a7ce98ab3d4819b7565620e_the_mana_pocket_patch_1024x1024.png';
  function mount(){
    var footer=document.querySelector('.footer-section,.Footer,.footer');if(!footer||footer.dataset.mpReceiptReady)return;footer.dataset.mpReceiptReady='true';
    var links=Array.prototype.map.call(footer.querySelectorAll('a'),function(link){return{href:link.getAttribute('href')||'#',text:link.textContent.trim(),className:link.className};});var navLinks=links.filter(function(item){return!/social-icon/.test(item.className)&&item.text;});var socials=links.filter(function(item){return/social-icon/.test(item.className);}).map(function(item){return{href:item.href,label:/facebook/.test(item.className)?'Facebook':/instagram/.test(item.className)?'Instagram':'Whatnot'};});
    footer.innerHTML='';footer.classList.add('mp-receipt-footer');footer.setAttribute('role','contentinfo');var surface=document.createElement('div');surface.className='mp-receipt-surface';var receipt=document.createElement('div');receipt.className='mp-receipt';
    var head=document.createElement('div');head.className='mp-receipt-head';var logo=document.createElement('img');logo.src=BRAND_LOGO;logo.alt='';head.appendChild(logo);var title=document.createElement('div');title.innerHTML='<strong>THE MANA POCKET</strong><span>CARDS \u2022 COMICS \u2022 COLLECTIBLES</span>';head.appendChild(title);receipt.appendChild(head);
    var meta=document.createElement('div');meta.className='mp-receipt-meta';meta.setAttribute('aria-hidden','true');meta.innerHTML='<span>COLLECTOR COPY</span><span>COLLECTOR: YOU</span>';receipt.appendChild(meta);
    var rows=document.createElement('nav');rows.className='mp-receipt-rows';rows.setAttribute('aria-label','Footer');navLinks.forEach(function(item){var link=document.createElement('a');link.href=item.href;var label=document.createElement('span');label.textContent=item.text.toUpperCase();var arrow=document.createElement('b');arrow.textContent='\u2192';link.appendChild(label);link.appendChild(arrow);rows.appendChild(link);});receipt.appendChild(rows);
    if(socials.length){var social=document.createElement('div');social.className='mp-receipt-social';var label=document.createElement('strong');label.textContent='FOLLOW THE POCKET';social.appendChild(label);var list=document.createElement('div');socials.forEach(function(item){var link=document.createElement('a');link.href=item.href;link.textContent=item.label;link.target='_blank';link.rel='noopener';list.appendChild(link);});social.appendChild(list);receipt.appendChild(social);}
    var thanks=document.createElement('div');thanks.className='mp-receipt-thanks';thanks.innerHTML='<strong>THANKS FOR STOPPING IN.</strong><span>Fast shipping \u2022 Expert packing \u2022 Collector-grade care</span>';receipt.appendChild(thanks);var barcode=document.createElement('div');barcode.className='mp-receipt-barcode';barcode.setAttribute('aria-hidden','true');receipt.appendChild(barcode);var legal=document.createElement('small');legal.textContent='\u00a9 '+new Date().getFullYear()+' The Mana Pocket';receipt.appendChild(legal);surface.appendChild(receipt);footer.appendChild(surface);
  }
  if(document.body)mount();else document.addEventListener('DOMContentLoaded',mount,{once:true});
}
...
function polishShopInventory(){
  ...
  function subtype(card,kind){
    var explicit=card.getAttribute('data-mp-subcategory');if(explicit)return explicit;
    var words=String(card.textContent||'').toLowerCase();
    if(kind==='comics')return /preorder/.test(words)?'preorders':/manga|graphic novel|trade paperback|omnibus|hardcover|\bvol\.?\b/.test(words)?'graphic-novels':'in-stock';
    if(kind==='collectibles')return /plush|squishmallow/.test(words)?'plush':/shirt|hoodie|hat|cap|apparel/.test(words)?'apparel':'figures';
    if(kind==='supplies')return /sleeve/.test(words)?'sleeves':/binder/.test(words)?'binders':/top.?loader/.test(words)?'toploaders':/playmat/.test(words)?'playmats':'other-supplies';
    if(kind==='pokemon'||kind==='mtg'||kind==='sports-cards')return /psa|bgs|cgc|sgc|graded|slab/.test(words)?'graded':/booster|sealed|box|pack|bundle|elite trainer|\betb\b|tin|deck/.test(words)?'sealed':'singles';
    return'all';
  }
  function subtypeOptions(kind){
    var sets={
      'sports-cards':[['all','All sports cards'],['singles','Singles'],['sealed','Sealed product'],['graded','Graded cards']],
      pokemon:[['all','All Pokémon'],['singles','Singles'],['sealed','Sealed product'],['graded','Graded cards']],
      mtg:[['all','All MTG'],['singles','Singles'],['sealed','Sealed product'],['graded','Graded cards']],
      comics:[['all','All comics'],['in-stock','In-stock comics'],['preorders','Comic preorders'],['graphic-novels','Graphic novels & manga']],
      collectibles:[['all','All collectibles'],['plush','Plush'],['figures','Figures & toys'],['apparel','Apparel']],
      supplies:[['all','All supplies'],['sleeves','Sleeves'],['binders','Binders'],['toploaders','Top loaders'],['playmats','Playmats']]
  ...
  function mount(){
    ...
    var sub=document.createElement('select');sub.className='mp-shop-subcategory';sub.setAttribute('aria-label','Product type');select.insertAdjacentElement('afterend',sub);select.closest('.wo-store-controls').classList.add('mp-shop-controls-expanded');
    var params=new URLSearchParams(location.search);var requested=canonical(params.get('cat'));var requestedSubtype=String(params.get('type')||params.get('subcat')||'all');var search=host.querySelector('.wo-store-search-field');var requestedSearch=String(params.get('q')||params.get('search')||'').trim();
    if(requested!=='all')select.value=requested;
    if(search&&requestedSearch)search.value=requestedSearch;
    function populateSubtype(){var options=subtypeOptions(select.value);sub.innerHTML=options.map(function(option){return'<option value="'+option[0]+'">'+option[1]+'</option>';}).join('');sub.hidden=select.value==='all';if(options.some(function(option){return option[0]===requestedSubtype;}))sub.value=requestedSubtype;else sub.value='all';requestedSubtype='all';}
    function apply(){
      var chosen=select.value,chosenSubtype=sub.value||'all';var query=String(search&&search.value||'').trim().toLowerCase();
      Array.prototype.forEach.call(host.querySelectorAll('.wo-live-card'),function(card){
        ...
        var subtypeMatch=chosenSubtype==='all'||subtype(card,kind)===chosenSubtype;
        card.style.display=categoryMatch&&subtypeMatch&&(!query||name.indexOf(query)!==-1)?'':'none';
      });
    }
    populateSubtype();select.addEventListener('change',function(){populateSubtype();window.setTimeout(apply,0);});sub.addEventListener('change',function(){window.setTimeout(apply,0);});if(search)search.addEventListener('input',function(){window.setTimeout(apply,0);});apply();
...
function loadHomepageRedesign(){
  ...
  script.async=true;script.setAttribute('data-mp-home-redesign','');
