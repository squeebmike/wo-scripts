...
function mount(catalog){
  var host=document.getElementById('wo-live-shop'),controls=host&&host.querySelector('.wo-store-controls'),inventoryGrid=controls&&controls.nextElementSibling;if(!inventoryGrid)return false;if(host.querySelector('.mp-shop-preorder-grid'))return true;
  ...
  var select=controls.querySelector('.wo-store-control-field'),sub=controls.querySelector('.wo-store-type-field')||controls.querySelector('.mp-shop-subcategory'),search=controls.querySelector('input');function sync(){window.setTimeout(function(){var category=String(select&&select.value||'all').toLowerCase(),subtype=String(sub&&sub.value||'all'),query=String(search&&search.value||'').trim().toLowerCase(),showCategory=(category==='all'||category==='comics'||category==='comic')&&(subtype==='all'||subtype==='preorders'),any=false;Array.prototype.forEach.call(grid.querySelectorAll('[data-preorder-sku]'),function(card){var visible=showCategory&&(!query||card.textContent.toLowerCase().includes(query));card.hidden=!visible;if(visible)any=true;});grid.hidden=!showCategory||(!any&&rendered>0);},20);}if(select)select.addEventListener('change',sync);if(sub)sub.addEventListener('change',sync);if(search)search.addEventListener('input',sync);sync();return true;
}
...
(function waitForInventory(){var host=document.getElementById('wo-live-shop'),inventoryGrid=host&&host.querySelector('.wo-live-grid');if(!host||!inventoryGrid||host.dataset.woLoaded!=='true'){setTimeout(waitForInventory,125);return;}lazyTrigger=document.createElement('button');lazyTrigger.type='button';lazyTrigger.className='mp-shop-preorder-more mp-shop-preorder-lazy-trigger';lazyTrigger.textContent='Load comic preorders';lazyTrigger.addEventListener('click',loadInitialPreorders);host.appendChild(lazyTrigger);if('IntersectionObserver' in window){var observer=new IntersectionObserver(function(entries){if(entries[0]&&entries[0].isIntersecting){observer.disconnect();loadInitialPreorders();}},{rootMargin:'900px 0px'});observer.observe(lazyTrigger);}})();
})();
