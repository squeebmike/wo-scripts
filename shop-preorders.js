...
function closeDetails(){var node=document.querySelector('.mp-shop-preorder-overlay');if(node)node.remove();document.removeEventListener('keydown',escapeDetails);window.removeEventListener('popstate',closeDetails);}
...
function mount(catalogs){
  ...
  var heading=document.createElement('section');heading.className='mp-shop-preorder-heading';heading.innerHTML='<div><span>ORDER AHEAD · ALL OPEN FOC WEEKS</span><h2>Comic preorders</h2><p>Pick the cover you want from every open preorder week and pay before that cover\'s deadline. Ratio incentives stay request-only until secured.</p></div><a href="/preorders">Browse by FOC week →</a>';
  var batchSize=window.matchMedia&&window.matchMedia('(max-width: 767px)').matches?8:12,rendered=0,more=document.createElement('button'),observer=null,total=sources.reduce(function(sum,source){return sum+source.total;},0)||variants.length,loading=false;
  ...
  function nextSource(){for(var i=0;i<sources.length;i++)if(sources[i].hasMore)return sources[i];return null;}
  function renderNext(){
    var next=variants.slice(rendered,rendered+(rendered===0?variants.length:batchSize)),source;
    if(!next.length&&(source=nextSource())&&!loading){loading=true;more.disabled=true;more.textContent='Loading more preorders…';fetch(API+'/public/preorders?store_id='+encodeURIComponent(STORE_ID)+'&cycle='+encodeURIComponent(source.cycleId)+'&distributor='+encodeURIComponent(source.distributor||'PRH')+'&limit='+batchSize+'&offset='+source.nextOffset,{headers:{Accept:'application/json'}}).then(function(response){if(!response.ok)throw new Error('Preorders unavailable');return response.json();}).then(function(page){addPage(page);source.hasMore=page.hasMore===true;source.nextOffset=Number(page.nextOffset)||source.nextOffset;loading=false;renderNext();}).catch(function(){loading=false;more.disabled=false;more.textContent='Try loading more preorders';});return;}
    ...
    more.textContent=(rendered<variants.length||nextSource())?'Show more preorders ('+rendered+' of '+total+')':'All '+total+' preorders shown';more.disabled=rendered>=variants.length&&!nextSource();
...
// distributor=PRH when no distributor param is sent (see foc-preorders.mjs),
// and this only ever called them with no distributor at all, so every
// Lunar FOC week was completely invisible on /shop even though the
// dedicated /preorders page already has a working PRH/Lunar toggle. Fetches
// both distributors' open weeks and merges them into one grid instead of a
// toggle -- /shop's comic-preorders section is meant to be "everything
// open right now", not a per-distributor view.
var PREORDER_DISTRIBUTORS=['PRH','Lunar'];
function loadInitialPreorders(){if(loadInitialPreorders.started)return;loadInitialPreorders.started=true;if(lazyTrigger){lazyTrigger.disabled=true;lazyTrigger.textContent='Loading comic preorders…';}Promise.all(PREORDER_DISTRIBUTORS.map(function(distributor){return fetch(API+'/public/preorders/weeks?summary=1&store_id='+encodeURIComponent(STORE_ID)+'&distributor='+encodeURIComponent(distributor),{headers:{Accept:'application/json'}}).then(function(response){if(!response.ok)throw new Error('Preorder weeks unavailable');return response.json();}).then(function(summary){return(summary.cycles||[]).filter(function(cycle){return cycle.isOpen;}).map(function(cycle){return{id:cycle.id,distributor:distributor};});}).catch(function(){return[];});})).then(function(openByDistributor){var open=[].concat.apply([],openByDistributor);if(!open.length)throw new Error('No preorder weeks are open');return Promise.all(open.map(function(cycle){return fetch(API+'/public/preorders?store_id='+encodeURIComponent(STORE_ID)+'&cycle='+encodeURIComponent(cycle.id)+'&distributor='+encodeURIComponent(cycle.distributor)+'&limit='+initialLimit+'&offset=0',{headers:{Accept:'application/json'}}).then(function(response){if(!response.ok)throw new Error('Preorders unavailable');return response.json();});}));}).then(function(catalogs){var tries=0;function tryMount(){if(mount(catalogs))return;if(++tries<80)setTimeout(tryMount,125);}tryMount();}).catch(function(error){loadInitialPreorders.started=false;if(lazyTrigger){lazyTrigger.disabled=false;lazyTrigger.textContent='Try loading comic preorders';}console.warn('[Mana Pocket] Comic preorders could not be added to the shop:',error.message);});}
