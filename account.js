...
function pickListCardHtml(list){
  ...
  return '<article class="mp-acct-order mp-acct-saved-pulls" data-pick-list="'+esc(list.id)+'"><header><div><h3>'+items.reduce(function(n,item){return n+Number(item.quantity||0);},0)+' saved comic'+(items.length===1?'':'s')+'</h3><span class="mp-acct-sub">Preorder deadline '+esc(dateLabel(cycle.foc_date))+' · '+(list.isOpen?'open':'expired — ordering disabled')+'</span></div><span class="mp-acct-status-pill">unpaid</span></header>'+items.map(function(item){var sku=item.sku||{};return'<div class="mp-acct-item-line mp-acct-pick-line" data-saved-sku="'+esc(sku.id||item.sku_id||'')+'"><label class="mp-acct-pick-check"><input type="checkbox" data-pick-select="'+esc(sku.id||item.sku_id||'')+'" '+(list.isOpen?'':'disabled')+'><span class="sr-only">Select '+esc(sku.title||'comic')+'</span></label>'+(sku.cover_image_url?'<img src="'+esc(sku.cover_image_url)+'" alt="">':'<span class="mp-acct-item-noimg"></span>')+'<div class="mp-acct-item-info"><span>'+esc(sku.title||'')+(sku.variant_label?' · '+esc(sku.variant_label):'')+' ×'+Number(item.quantity||1)+'</span><span>'+money(Number(sku.customer_price_cents||0)/100)+' each · releases '+esc(dateLabel(sku.on_sale_date))+'</span></div><div class="mp-acct-pick-actions">'+(list.isOpen?'<button class="mp-acct-mini" type="button" data-pick-add="'+esc(sku.id||item.sku_id||'')+'">Add to cart</button><button class="mp-acct-mini primary" type="button" data-pick-pay="'+esc(sku.id||item.sku_id||'')+'">Pay for this</button>':'')+'<button class="mp-acct-mini danger" type="button" data-pick-remove="'+esc(sku.id||item.sku_id||'')+'">Remove</button></div></div>';}).join('')+'<div class="mp-acct-row"><span>Saved-list subtotal</span><strong>'+money(total/100)+'</strong></div><div class="mp-acct-actions">'+(list.isOpen?'<button class="mp-acct-button ghost" type="button" data-pick-list-add="'+esc(list.id)+'">Add this week to cart</button><button class="mp-acct-button" type="button" data-pick-list-pay="'+esc(list.id)+'">Pay for this week</button>':'')+'<a class="mp-acct-button ghost" href="/preorders?cycle='+esc(cycle.id||'')+'">View this week</a><span class="mp-acct-action-status" aria-live="polite"></span></div></article>';
}

function findSavedPick(skuId){var lists=state.cache.preorders&&state.cache.preorders.picks||[];for(var i=0;i<lists.length;i++){for(var j=0;j<(lists[i].items||[]).length;j++){var item=lists[i].items[j],sku=item.sku||{};if((sku.id||item.sku_id)===skuId)return{list:lists[i],item:item};}}return null;}
function savedPickLine(record){var list=record.list,item=record.item,cycle=list.cycle||{},sku=item.sku||{},skuId=sku.id||item.sku_id;return{id:'foc:'+skuId,kind:'preorder',skuId:skuId,cycleId:cycle.id,focDate:cycle.foc_date,name:[sku.title,sku.variant_label].filter(Boolean).join(' · ')||'Saved comic pull',image:sku.cover_image_url||'',upc:sku.upc||'',price:Number(sku.customer_price_cents||0)/100,available:50,qty:Math.max(1,Math.min(50,Number(item.quantity||1)))};}
function savedLines(test){var lines=[];(state.cache.preorders&&state.cache.preorders.picks||[]).forEach(function(list){if(!list.isOpen)return;(list.items||[]).forEach(function(item){var record={list:list,item:item};if(!test||test(record))lines.push(savedPickLine(record));});});return lines;}
function addSavedLinesToCart(lines){if(!window.WO||typeof window.WO.getCart!=='function'||typeof window.WO.setCart!=='function')throw new Error('The cart is still loading. Please try again in a moment.');var cart=window.WO.getCart(),byId={};cart.forEach(function(line){byId[line.id]=line;});lines.forEach(function(line){if(byId[line.id])byId[line.id].qty=line.qty;else{cart.push(line);byId[line.id]=line;}});window.WO.setCart(cart);}
function paySavedLines(lines,out){if(!lines.length)throw new Error('Select at least one open comic to pay for.');if(!window.WO||typeof window.WO.checkoutPreorderLines!=='function')throw new Error('Secure checkout is still loading. Please try again in a moment.');if(out)out.textContent='Opening secure checkout…';window.WO.checkoutPreorderLines(lines,function(){state.cache.preorders=null;loadPreorders();});}
function preorderCardHtml(order){
  ...
  var cancelLabel='Remove unpaid attempt';
...
async function handlePreorderAction(event){
  var addOne=event.target.closest('[data-pick-add]'),payOne=event.target.closest('[data-pick-pay]'),removeOne=event.target.closest('[data-pick-remove]'),addList=event.target.closest('[data-pick-list-add]'),payList=event.target.closest('[data-pick-list-pay]'),addAll=event.target.closest('[data-picks-add-all]'),paySelected=event.target.closest('[data-picks-pay-selected]');
  if(addOne||payOne||removeOne||addList||payList||addAll||paySelected){
    var savedCard=event.target.closest('.mp-acct-saved-pulls'),savedOut=(savedCard&&savedCard.querySelector('.mp-acct-action-status'))||document.querySelector('[data-picks-status]');
    try{
      if(removeOne){var removeId=removeOne.dataset.pickRemove;if(!window.confirm('Remove this comic from your saved pulls? It will also be removed from the cart.'))return;removeOne.disabled=true;if(savedOut)savedOut.textContent='Removing saved pull…';await api('/public/preorders/picks',{method:'DELETE',body:JSON.stringify({storeId:STORE_ID,skuIds:[removeId]})});if(window.WO&&typeof window.WO.getCart==='function'&&typeof window.WO.setCart==='function')window.WO.setCart(window.WO.getCart().filter(function(line){return line.id!=='foc:'+removeId;}));state.cache.preorders=null;await loadPreorders();return;}
      var lines=[];
      if(addOne||payOne){var one=findSavedPick((addOne||payOne).getAttribute(addOne?'data-pick-add':'data-pick-pay'));if(one&&one.list.isOpen)lines=[savedPickLine(one)];}
      else if(addList||payList){var listId=(addList||payList).getAttribute(addList?'data-pick-list-add':'data-pick-list-pay');lines=savedLines(function(record){return record.list.id===listId;});}
      else if(addAll)lines=savedLines();
      else if(paySelected){var selected={};document.querySelectorAll('[data-pick-select]:checked').forEach(function(input){selected[input.dataset.pickSelect]=true;});lines=savedLines(function(record){var sku=record.item.sku||{};return selected[sku.id||record.item.sku_id];});}
      if(!lines.length)throw new Error('Select at least one open comic.');
      if(addOne||addList||addAll){addSavedLinesToCart(lines);if(savedOut)savedOut.textContent=lines.length+' saved comic'+(lines.length===1?'':'s')+' added to your cart. Your saved list is unchanged.';}
      else paySavedLines(lines,savedOut);
    }catch(error){if(savedOut){savedOut.textContent=error.message;savedOut.classList.add('error');}else window.alert(error.message);}
    return;
  }
  ...
  if(cancel&&!window.confirm('Remove this unpaid preorder attempt? This cannot be undone.'))return;
