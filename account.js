...
async function loadPreorders(){
  ...
  try{
    ...
    var saved=picks.length?'<div class="mp-acct-saved-head"><div><h2 class="mp-acct-subhead">Saved comic pulls</h2><p class="mp-acct-intro">This is your curated pull list. Removing a comic from the cart does not remove it here. Add everything to the cart, select only what you want to pay for, or remove a saved pull explicitly.</p></div><div class="mp-acct-actions"><button class="mp-acct-button ghost" type="button" data-picks-add-all>Add all to cart</button><button class="mp-acct-button" type="button" data-picks-pay-selected>Pay selected</button></div></div><div class="mp-acct-action-status" data-picks-status aria-live="polite"></div>'+picks.map(pickListCardHtml).join(''):'';
...
function pickListCardHtml(list){
  ...
  return '<article class="mp-acct-order mp-acct-saved-pulls" data-pick-list="'+esc(list.id)+'"><header><div><h3>'+items.reduce(function(n,item){return n+Number(item.quantity||0);},0)+' saved pull'+(items.length===1?'':'s')+'</h3><span class="mp-acct-sub">FOC '+esc(dateLabel(cycle.foc_date))+' · '+(list.isOpen?'ready when you are':'deadline closed')+'</span></div><span class="mp-acct-status-pill">unpaid</span></header>'+items.map(function(item){var sku=item.sku||{};return'<div class="mp-acct-item-line mp-acct-pick-line" data-saved-sku="'+esc(sku.id||item.sku_id||'')+'"><label class="mp-acct-pick-check"><input type="checkbox" data-pick-select="'+esc(sku.id||item.sku_id||'')+'" '+(list.isOpen?'':'disabled')+'><span class="sr-only">Select '+esc(sku.title||'comic')+'</span></label>'+(sku.cover_image_url?'<img src="'+esc(sku.cover_image_url)+'" alt="">':'<span class="mp-acct-item-noimg"></span>')+'<div class="mp-acct-item-info"><span>'+esc(sku.title||'')+(sku.variant_label?' · '+esc(sku.variant_label):'')+' ×'+Number(item.quantity||1)+'</span><span>'+money(Number(sku.customer_price_cents||0)/100)+' each</span></div><div class="mp-acct-pick-actions">'+(list.isOpen?'<button class="mp-acct-mini" type="button" data-pick-add="'+esc(sku.id||item.sku_id||'')+'">Add to cart</button><button class="mp-acct-mini primary" type="button" data-pick-pay="'+esc(sku.id||item.sku_id||'')+'">Pay this</button>':'')+'<button class="mp-acct-mini danger" type="button" data-pick-remove="'+esc(sku.id||item.sku_id||'')+'">Remove</button></div></div>';}).join('')+'<div class="mp-acct-row"><span>Saved-list subtotal</span><strong>'+money(total/100)+'</strong></div><div class="mp-acct-actions">'+(list.isOpen?'<button class="mp-acct-button ghost" type="button" data-pick-list-add="'+esc(list.id)+'">Add this FOC to cart</button><button class="mp-acct-button" type="button" data-pick-list-pay="'+esc(list.id)+'">Pay this FOC</button>':'')+'<a class="mp-acct-button ghost" href="/preorders?cycle='+esc(cycle.id||'')+'">Browse this FOC</a><span class="mp-acct-action-status" aria-live="polite"></span></div></article>';
}
