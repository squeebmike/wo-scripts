...
var state={cycles:null,cart:readJson(CART_KEY,{}),session:readJson(SESSION_KEY,null),filters:{q:'',publisher:'all',artist:'all',kind:'all'},timer:null,deepLinkHandled:false};
...
function saveCart(){saveJson(CART_KEY,state.cart);renderFloat();}
...
function mount(){
  ...
  if(!document.getElementById('navbarID')){
    ...
    nav.innerHTML='<a class="mp-foc-brand" href="/" aria-label="The Mana Pocket home"><img src="https://cdn.prod.website-files.com/65b15ee0228d06647ca7e4ce/6a7ce98ab3d4819b7565620e_the_mana_pocket_patch_1024x1024.png" alt="The Mana Pocket"></a><div><a href="/">Home</a><a href="/shop">Shop</a><button type="button" data-foc-theme>Pick a theme</button></div>';
    document.body.prepend(nav);nav.querySelector('[data-foc-theme]').addEventListener('click',function(){if(window.WO&&typeof window.WO.openTheme==='function')window.WO.openTheme();});
  }
...
async function loadCatalog(){
  try{var data=await api('/public/preorders/weeks?store_id='+encodeURIComponent(STORE_ID),{auth:false});state.cycles=(data.cycles||[]).slice().sort(function(a,b){return new Date(a.cycle.customer_cutoff_at)-new Date(b.cycle.customer_cutoff_at);});render();handleDeepLink();}
...
function renderFloat(){var button=document.querySelector('[data-cart]');if(button)button.innerHTML='My picks <span>'+cartLines().reduce(function(n,l){return n+l.quantity;},0)+'</span>';}
...
function statusWithResend(node,message,email,kind){
  ...
  button.addEventListener('click',function(){
    ...
    auth('resend',{type:'signup',email:email}).then(function(){
      node.innerHTML='<div class="mp-foc-status success">Confirmation email resent — check your inbox (and spam folder).</div>';
...
function openCart(){
  ...
  var html='<h2>My exact-cover picks</h2><div data-cart-lines>'+linesHtml+'</div><div class="mp-foc-cart-total"><span>Books subtotal</span><span>'+money(cartTotal())+'</span></div>'+(groups.length>1?'<p class="mp-foc-status">Picks span '+groups.length+' FOC weeks — you\'ll check out for each week separately.</p>':'')+'<div class="mp-foc-cart-actions"><button class="mp-foc-button ghost" data-close-cart>Keep looking</button><button class="mp-foc-button" data-checkout '+(!groups.length?'disabled':'')+'>Secure these covers</button></div>';
