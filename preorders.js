...
// it always has.
var state={distributor:'PRH',cycles:null,session:readJson(SESSION_KEY,null),filters:{q:'',publisher:'all',artist:'all',kind:'all',type:'all'},timer:null,cycleObserver:null,lazyReady:false,lastLazyScrollY:0,scrollBound:false,deepLinkHandled:false,loadingCycles:new Set()};
...
function filteredFamiliesFor(entry){
  // family.comicType is PRH's own ComicType column, already flowing through
  // loadCatalog -- carries far more than "comic": store report confirmed
  // real weekly FOC files mix in posters, non-book items, card/board games,
  // and boxed sets alongside actual issues under distinct ComicType values
  // ("Non-Book Item" for the poster/facsimile rows spotted live), with no
  // way to tell them apart while browsing. Built from unique() the same way
  // publishers/artists already are, so it always reflects whatever real
  // ComicType values are actually in this week's file instead of a
  // hardcoded guess at PRH's category list.
...
function render(){
  ...
  var types=unique(state.cycles.flatMap(function(e){return(e.families||[]).map(function(f){return f.comicType||'Comic';});}));
...
function coverHtml(family,sku,cycleIsOpen){
  var image=sku.coverImageUrl?'<img loading="lazy" decoding="async" src="'+esc(sku.coverImageUrl)+'" alt="'+esc((family.seriesName||family.title)+' '+sku.variantLabel)+'">':'<span>No cover image yet</span>';
  var ratio=sku.isIncentive?'<div class="mp-foc-ratio">'+esc(sku.orderRequirement||('1:'+sku.ratioThreshold+' incentive'))+'<div class="mp-foc-progress"><i style="width:'+Math.min(100,100*Number(sku.qualification&&sku.qualification.total||0)/Math.max(1,Number(sku.ratioThreshold||1)))+'%"></i></div>'+(sku.waitlistOnly?'Request list only · no charge':'Secured copies available')+'</div>':'';
  var action=!cycleIsOpen?'Preorder closed':sku.waitlistOnly?'Request one':(sku.canPreorder?'Preorder this cover':'Price coming soon');
...
function addPreorderLine(skuId,qty,sourceEl){
  ...
  else{var newLine=preorderLineFor(match,add);cart.push(newLine);}
  if(window.WO&&typeof window.WO.setCart==='function')window.WO.setCart(cart);
  var savedQuantity=existing?existing.qty:newLine.qty;
  savePick(skuId,savedQuantity);
  trackPreorderEvent('preorder_saved',{currency:'USD',value:Number(match.sku.priceCents||0)/100*savedQuantity,items:[preorderAnalyticsItem(match,savedQuantity)]});
...
function openSkuDetail(skuId){var match=findSku(skuId);if(!match)return;trackPreorderEvent('view_item',{currency:'USD',value:Number(match.sku.priceCents||0)/100,items:[preorderAnalyticsItem(match,1)]});bindSkuDetail(dialog(skuDetailHtml(match.family,match.sku,match.cycle),'wide'),match);}
...
function checkoutPreorderLines(lines,onAllDone){trackPreorderEvent('preorder_checkout_started',{currency:'USD',value:lines.reduce(function(sum,line){return sum+Number(line.price||0)*Number(line.qty||1);},0),items:lines.map(function(line){return{item_id:String(line.skuId||line.id||''),item_name:String(line.name||'Comic preorder'),item_category:'Comic preorder',price:Number(line.price||0),quantity:Number(line.qty||1)};})});requireSession(function(){preorderCheckoutQueue(groupPreorderLinesByCycle(lines),0,onAllDone);});}
